...
cp -v run.sh deploy.sh fresh_checkout.sh ghu.service nginx.conf "$deploy_root"
...
worktree_current_old_old=$(readlink -f "$worktree_current_old" || true)
